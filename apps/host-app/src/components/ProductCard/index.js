...
export default function ProductCard({
    ...
    const {
        addToBasket: addToBasketLocal,
        updateQuantity: updateQuantityLocal,
        removeFromBasket: removeFromBasketLocal,
        isProductInBasket,
        getProductQuantityInBasket
    } = useProductStore()

    // Remote basket integration
    const {
        addToBasket: addToBasketRemote,
        updateQuantity: updateQuantityRemote,
        removeFromBasket: removeFromBasketRemote
    } = useBasketIntegration()
    ...
    const handleQuantityChange = (newQuantity) => {
        if (newQuantity === 0) {
            // Hem local'dan hem remote'dan çıkar
            removeFromBasketLocal(product.id)
            removeFromBasketRemote(product.id)
            message.info(SUCCESS_MESSAGES.PRODUCT_REMOVED)
        } else {
            // Hem local'da hem remote'da güncelle
            updateQuantityLocal(product.id, newQuantity)
            updateQuantityRemote(product.id, newQuantity)
            ...
            >
                <div style={{ flex: 1 }}>
                    ...
                    <Space direction="vertical" style={{ width: '100%' }} size="small">
                        {/* Rating */}
                        <Space size="small">
                            <Rate disabled defaultValue={product.rating?.rate || 0} allowHalf size="small" />
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            <div style={{ flex: 1 }}>
                                ...
                                <Space size="middle">
                                    <Rate disabled defaultValue={product.rating?.rate || 0} allowHalf size="small" />
                                    <Text type="secondary">
                                        ({product.rating?.count || 0} değerlendirme)
                                    </Text>
                                </Space>
                            </div>
